...
import { saveSearch, deleteSearch, savePlay, clearSearch, saveFavorite, deleteFavorite } from 'common/js/cache'
...
export const insertSong = function({
    ...
    // slice方法返回副本，不会直接修改原值，否则会报错
    let playlist = state.playlist.slice()
    let sequencelist = state.sequenceList.slice()
        // currentIndex是值类型，修改currentIndex不会影响原来的currentIndex，因此不需要slice
    let currentIndex = state.currentIndex
        // 记录当前歌曲
        // 查找当前播放列表中 是否存在  待插入的歌曲 并返回其索引
        // 因为是插入歌曲 所以索引➕1
        // 插入这首歌 到当前索引的位置
        // 如果包含这首歌
        // 如果插入的序号 大于列表中的序号
...
export const saveSearchHistory = function({
    ...
    // saveSearch(query)是存储历史数据到localStorage，并返回历史查询数据
...
export const saveFavoriteList = function({
    ...
    commit(types.SET_FAVORAITELIST, saveFavorite(song))
}

export const deleteFavoriteList = function({
    commit
}, song) {
    commit(types.SET_FAVORAITELIST, deleteFavorite(song))
}
